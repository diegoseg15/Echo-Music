...
object YTPlayerUtils {
                    ...
                    qobuzAttempt =
                        kotlinx.coroutines.withTimeoutOrNull(15000L) {
                            ...
                            if (title != null && author != null) {
                                val qobuzClient =
                                    iad1tya.echo.music.utils.qobuz
                                        .QobuzApiClient()
                        ...
                        android.os.Handler(android.os.Looper.getMainLooper()).post {
                            if (isDownload) {
                                android.widget.Toast
                                    .makeText(
                                        it,
                                        "Lossless download unavailable, falling back to Saavn (320kbps)",
                                ...
                                android.widget.Toast
                                    .makeText(
                                        it,
                                        "Lossless stream unavailable, falling back to Saavn (320kbps)",
                                        android.widget.Toast.LENGTH_LONG,
                                    ).show()
                            }
                        }
            ...
            try {
                saavnAttempt =
                    kotlinx.coroutines.withTimeoutOrNull(15000L) {
                        ...
                        val songs =
                            com.music.jiosaavn.SaavnService
                                .searchSongs(query)
                                .getOrNull()
                        ...
                        val streamUrl =
                            com.music.jiosaavn.SaavnService
                                .getBestStreamUrl(bestSong.id, "320kbps")
                ...
                context?.let {
                    if (showFallbackToast && !hasShownSaavnToast) {
                        ...
                        android.os.Handler(android.os.Looper.getMainLooper()).post {
                            ...
                            android.widget.Toast
                                .makeText(it, msg, android.widget.Toast.LENGTH_SHORT)
                                .show()
                        }
                    }
                }
    ...
    private suspend fun resolvePlaybackData(
        ...
        runCatching {
            ...
            for (clientIndex in (startIndex until STREAM_FALLBACK_CLIENTS.size)) {
                ...
                if (clientIndex == -1) {
                    ...
                    PlaybackLogManager.log(
                        ...
                        "Trying fallback [${clientIndex + 1}/${STREAM_FALLBACK_CLIENTS.size}]",
                        client.clientName,
                    )
                ...
                if (streamPlayerResponse?.playabilityStatus?.status == "OK") {
                    Timber
                        .tag(
                            ...
                            "Player response status OK for client: ${if (clientIndex == -1) {
                    ...
                    PlaybackLogManager.log(
                        ...
                        if (clientIndex ==
                            ...
                            STREAM_FALLBACK_CLIENTS[clientIndex].clientName
                        },
                    )

                    val hasDirectUrls =
                        streamPlayerResponse.streamingData
                            ?.adaptiveFormats
                            ?.any { !it.url.isNullOrEmpty() } == true
                    val hasSignatureCipher =
                        streamPlayerResponse.streamingData
                            ?.adaptiveFormats
                    ...
                    if (format == null) {
                        Timber
                            .tag(
                                ...
                                "No suitable format found for client: ${if (clientIndex == -1) {
                                    MAIN_CLIENT.clientName
                                } else {
                                    STREAM_FALLBACK_CLIENTS[clientIndex]
                                        .clientName
            ...
            if (isUploadedTrack) {
                println(
                    "[PLAYBACK_DEBUG] SUCCESS: Got playback data for uploaded track - format=${format.mimeType}, streamUrl=${streamUrl?.take(
                        100,
                    )}...",
                )
            }
    ...
    suspend fun playerResponseForMetadata(
        ...
        return YouTube
            .player(videoId, playlistId, client = WEB_REMIX)
    ...
    private fun findFormat(
        ...
        fun PlayerResponse.StreamingData.Format.isAacMp4(): Boolean =
            mimeType.startsWith("audio/mp4", ignoreCase = true) ||
                mimeType.contains("mp4a.40.2", ignoreCase = true)

        fun PlayerResponse.StreamingData.Format.isWebmOpus(): Boolean =
            mimeType.startsWith("audio/webm", ignoreCase = true) ||
                mimeType.contains("opus", ignoreCase = true)

        fun PlayerResponse.StreamingData.Format.scoreNear(targetBitrate: Int): Int {
            val distance =
...
private fun artistSimilarity(
    ...
    val hasDistinctiveOverlap =
        intersection.any { token ->
...
fun confidence(
    ...
    queryTitle: String,
    queryDuration: Long?,
    candidate: iad1tya.echo.music.utils.qobuz.QobuzTrack,
): Float {

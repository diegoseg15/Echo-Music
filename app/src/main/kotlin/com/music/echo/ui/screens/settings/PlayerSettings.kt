...
fun PlayerSettings(
    ...
    val (wifiAudioQualityProfile, onWifiAudioQualityProfileChange) =
        rememberEnumPreference(
            AudioQualityWifiProfileKey,
            defaultValue = PlaybackAudioQualityProfile.HIGH,
        )

    val (mobileAudioQualityProfile, onMobileAudioQualityProfileChange) =
        rememberEnumPreference(
            AudioQualityMobileProfileKey,
            defaultValue = PlaybackAudioQualityProfile.MEDIUM,
        )

    val (batteryAudioQualityProfile, onBatteryAudioQualityProfileChange) =
        rememberEnumPreference(
            AudioQualityBatteryProfileKey,
            defaultValue = PlaybackAudioQualityProfile.LOW,
        )
    val (showAudioFallbackToast, onShowAudioFallbackToastChange) =
        rememberPreference(
    ...
    val (pauseOnMute, onPauseOnMuteChange) =
        rememberPreference(
            PauseOnMute,
            defaultValue = false,
        )
    ...
    val (keepScreenOn, onKeepScreenOnChange) =
        rememberPreference(
            KeepScreenOn,
            defaultValue = false,
        )
    ...
    var showWifiAudioQualityProfileDialog by remember {
        mutableStateOf(false)
    }

    var showMobileAudioQualityProfileDialog by remember {
        mutableStateOf(false)
    }

    var showBatteryAudioQualityProfileDialog by remember {
        mutableStateOf(false)
    }

    val audioSourceYoutubeOpusText = stringResource(R.string.audio_source_youtube_opus)
    val audioSourceSaavnText = stringResource(R.string.audio_source_saavn)
    val audioSourceQobuzLosslessText = stringResource(R.string.audio_source_qobuz_lossless)

    val playbackQualityAutoText = stringResource(R.string.playback_quality_auto)
    val playbackQualityHighText = stringResource(R.string.playback_quality_high)
    val playbackQualityMediumText = stringResource(R.string.playback_quality_medium)
    val playbackQualityLowText = stringResource(R.string.playback_quality_low)

    fun audioQualityText(quality: AudioQuality): String =
        when (quality) {
            AudioQuality.OPUS -> audioSourceYoutubeOpusText
            AudioQuality.SAAVN -> audioSourceSaavnText
            AudioQuality.LOSSLESS -> audioSourceQobuzLosslessText
        }

    fun playbackQualityProfileText(profile: PlaybackAudioQualityProfile): String =
        when (profile) {
            PlaybackAudioQualityProfile.AUTO -> playbackQualityAutoText
            PlaybackAudioQualityProfile.HIGH -> playbackQualityHighText
            PlaybackAudioQualityProfile.MEDIUM -> playbackQualityMediumText
            PlaybackAudioQualityProfile.LOW -> playbackQualityLowText
        }
    ...
    if (showAudioQualityDialog) {
        EnumDialog(
            ...
            valueText = { audioQualityText(it) },
        )
    }

    if (showWifiAudioQualityProfileDialog) {
        EnumDialog(
            onDismiss = { showWifiAudioQualityProfileDialog = false },
            onSelect = {
                onWifiAudioQualityProfileChange(it)
                showWifiAudioQualityProfileDialog = false
            },
            title = stringResource(R.string.playback_quality_wifi),
            current = wifiAudioQualityProfile,
            values = PlaybackAudioQualityProfile.values().toList(),
            valueText = { playbackQualityProfileText(it) },
        )
    }

    if (showMobileAudioQualityProfileDialog) {
        EnumDialog(
            onDismiss = { showMobileAudioQualityProfileDialog = false },
            onSelect = {
                onMobileAudioQualityProfileChange(it)
                showMobileAudioQualityProfileDialog = false
            },
            title = stringResource(R.string.playback_quality_mobile),
            current = mobileAudioQualityProfile,
            values = PlaybackAudioQualityProfile.values().toList(),
            valueText = { playbackQualityProfileText(it) },
        )
    }

    if (showBatteryAudioQualityProfileDialog) {
        EnumDialog(
            onDismiss = { showBatteryAudioQualityProfileDialog = false },
            onSelect = {
                onBatteryAudioQualityProfileChange(it)
                showBatteryAudioQualityProfileDialog = false
            },
            title = stringResource(R.string.playback_quality_battery),
            current = batteryAudioQualityProfile,
            values = PlaybackAudioQualityProfile.values().toList(),
            valueText = { playbackQualityProfileText(it) },
        )
    }
    ...
    if (showDownloadQualityDialog) {
        EnumDialog(
            ...
            values =
                iad1tya.echo.music.constants.DownloadQuality
                    .values()
                    .toList(),
            valueText = {
                when (it) {
    ...
    Column(
        ...
        if (showSaavnAudioWarning) {
            DefaultDialog(
                ...
                title = { Text(stringResource(R.string.enable_saavn_audio_title)) },
                buttons = {
                ...
                Text(stringResource(R.string.enable_saavn_audio_message))
            }
        }
                    ...
                    TextButton(onClick = {
                        ...
                        if (crossfadeEnabled) {
                            ...
                            android.widget.Toast
                                .makeText(
                                    context,
                                    context.getString(R.string.crossfade_disabled_for_lossless),
                ...
                buildList {
                    add(
                        Material3SettingsItem(
                            ...
                            title = { Text(stringResource(R.string.audio_source)) },
                            description = {
                                Text(audioQualityText(audioQuality))
                            },
                    ...
                    add(
                        Material3SettingsItem(
                            ...
                            title = { Text(stringResource(R.string.playback_quality_wifi)) },
                            description = {
                                Text(playbackQualityProfileText(wifiAudioQualityProfile))
                            },
                            onClick = { showWifiAudioQualityProfileDialog = true },
                        ),
                    )
                    ...
                    add(
                        Material3SettingsItem(
                            icon = painterResource(R.drawable.linear_scale),
                            title = { Text(stringResource(R.string.playback_quality_mobile)) },
                            description = {
                                Text(playbackQualityProfileText(mobileAudioQualityProfile))
                            },
                            onClick = { showMobileAudioQualityProfileDialog = true },
                        ),
                    )
                    ...
                    add(
                        Material3SettingsItem(
                            icon = painterResource(R.drawable.linear_scale),
                            title = { Text(stringResource(R.string.playback_quality_battery)) },
                            description = {
                                Text(playbackQualityProfileText(batteryAudioQualityProfile))
                            },
                            onClick = { showBatteryAudioQualityProfileDialog = true },
                        ),
                    )
                    ...
                    add(
                        Material3SettingsItem(
                            ...
                            title = { Text(stringResource(R.string.audio_fallback_notifications_title)) },
                            description = {
                                Text(stringResource(R.string.audio_fallback_notifications_description))
                            },
                            trailingContent = {
                                Switch(
                        ...
                        Material3SettingsItem(
                            ...
                            description = {
                                if (isLosslessSelected) {
                                    Text(stringResource(R.string.crossfade_disabled_while_lossless))
                                } else {
                            ...
                            onClick = {
                                if (isLosslessSelected) {
                                    android.widget.Toast
                                        .makeText(
                                            context,
                                            "Crossfade is not available with Lossless audio",
                                            android.widget.Toast.LENGTH_SHORT,
                                        ).show()
